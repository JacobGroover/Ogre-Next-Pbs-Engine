...
# Finds a 3rd party DLL from the ogre-next-deps folder and copies it to the main binary output directory.
macro( findAndCopy3rdPartyDll BUILD_TYPE DLL_NAME )
    # Third-party DLLs are located in configuration-specific subfolders (Debug, Release, etc.)
    set( DEP_DLL_PATH "${CMAKE_SOURCE_DIR}/Dependencies/Ogre/ogre-next-deps/build/ogredeps/bin/${BUILD_TYPE}/${DLL_NAME}.dll" )

    if( EXISTS ${DEP_DLL_PATH} )
        # Copy the DLL to the main output directory where the executable resides.
        copyWithSymLink( ${DEP_DLL_PATH} "${CMAKE_SOURCE_DIR}/bin/${BUILD_TYPE}" )
    endif()
endmacro()
...
macro( setupPluginFileFromTemplate BUILD_TYPE OGRE_USE_SCENE_FORMAT OGRE_USE_PLANAR_REFLECTIONS )
    ...
    # Copy essential third-party DLLs like SDL2 and AMD AGS
		if( WIN32 )
			findAndCopy3rdPartyDll( ${BUILD_TYPE} "amd_ags_x64" )
			findAndCopy3rdPartyDll( ${BUILD_TYPE} "SDL2" )
		endif()
...
set( CMAKE_MODULE_PATH "${CMAKE_CURRENT_SOURCE_DIR}/Dependencies/Ogre/ogre-next/CMake/Packages" )

# Guess the paths.
set( OGRE_SOURCE "${CMAKE_CURRENT_SOURCE_DIR}/Dependencies/Ogre/ogre-next" CACHE STRING "Path to OGRE-Next source code (see http://www.ogre3d.org/tikiwiki/tiki-index.php?page=CMake+Quick+Start+Guide)" )
if( WIN32 )
